...
from data_generation.generate_data import GridWorldMDPDataGenerator, generate_random_trajectory, simulate_human_estop_v2
...
print_policy_2(policies[0], size)
...
for i in range(num_world):
    ...
    random_trajs = [generate_random_trajectory(env, max_horizon=size*size) for j in range(num_demonstration)]
    ...
    for demonstration in range(num_demonstration):
        ...
        birl = EBIRL(env, estops[:demonstration], beta)
