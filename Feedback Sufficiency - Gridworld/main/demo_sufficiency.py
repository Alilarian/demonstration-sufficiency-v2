...
for i in range(num_world):
    ...
    # Begin demonstration iteration
    for demonstration in range(num_demonstration):
        state, optimal_action = policies[i][demo_order[demonstration]]
        demos[i].append((state, optimal_action))
        
        logger.info(f"\nRunning BIRL with demonstration {demonstration+1}/{num_demonstration} in world {i+1}")
        logger.info(f"Current demos: {demos[i]}")
        
        # Initialize BIRL with demos
        birl = BIRL(env, demos[i], beta)
        birl.run_mcmc(num_steps, step_stdev, adaptive=adaptive)

        logger.info("MCMC sampling complete.")
        logger.info(f"Acceptance ratio: {birl.accept_rate}")
        ...
        logger.info(f"MAP Solution Reward Weights: {map_solution}")
        ...
        for alpha in alphas:
            ...
            logger.info(f"{alpha}-VaR bound for {demonstration + 1} demonstrations: {avar_bound:.6f}")
        
        # Calculate the true bound for the current map policy
        ...
        logger.info(f"True expected value difference for MAP policy: {true_bound:.6f}")

        # Evaluate threshold sufficiency for alpha=0.95
        for threshold in thresholds:
            avar_bound = avar_bounds[0.95][demonstration]
            logger.info(f"Evaluating threshold {threshold} with avar bound: {avar_bound}")
            
            if avar_bound[0] < threshold:
                logger.info(f"Threshold {threshold} SUFFICIENT with avar bound: {avar_bound[0]}")
                map_evd = true_bound
                bounds[threshold].append(avar_bound)
                num_demos[threshold].append(demonstration + 1)
                true_evds[threshold].append(map_evd)
                avg_bound_errors[threshold].append(avar_bound[0] - map_evd)
                ...
                logger.info(f"Threshold {threshold} INSUFFICIENT")
                if true_bound < threshold:
