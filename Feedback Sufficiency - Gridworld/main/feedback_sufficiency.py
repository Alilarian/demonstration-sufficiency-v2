...
parser.add_argument('--pair_wise_strategy', type=str, help='Pair-wise strategy to use', required=False)
...
pair_wise_strategy = args.pair_wise_strategy if args.pair_wise_strategy else config['data_generation']['pair_wise_strategy']
...
for i in range(num_world):
    ...
    pairwise_comparisons = GridWorldMDPDataGenerator(env=env, seed=seed).generate_pairwise_comparisons(strategy=pair_wise_strategy, num_trajs=num_demonstration)
    logger.info(f"Generated pairwise comparisons using strategy {pair_wise_strategy} for {num_demonstration} demonstrations.")
    ...
    for demonstration in range(num_demonstration):
        ...
        birl = PBIRL(env, pairwise_comparisons[:demonstration], beta)
