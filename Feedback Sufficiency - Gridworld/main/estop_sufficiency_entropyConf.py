...
for i in range(num_world ):
    ...
    if same_demonstration:
        ...
        selected_demo = random.choice(estops)
    ...
    for demonstration in range(num_demonstration):
        ...
        logger.info(f"\nRunning EBIRL with {demonstration + 1} demonstrations for experiment {i+1}")
        print(demos_shuffled[:demonstration+1])
        birl = EBIRL(env, demos_shuffled[:demonstration+1], beta)
