...
nnInfogain_thresholds  = [0.01, 0.03, 0.05, 0.07, 0.09, 0.11, 0.13, 0.15, 0.17, 0.2, 0.25, 0.30, 0.35, 0.4, 0.45]
...
########################################################################
# Metrics for nInfogainergence - all experiments
########################################################################
num_demos_nInfogain_all = []
pct_states_nInfogain_all = []
policy_optimalities_nInfogain_all = []
policy_accuracies_nInfogain_all = []
accuracies_nInfogain_all = []
...
for i in range(50):
    ...
    ########################################################################
    num_demos_nInfogain = {threshold: [] for threshold in nnInfogain_thresholds}
    pct_states_nInfogain = {threshold: [] for threshold in nnInfogain_thresholds}
    policy_optimalities_nInfogain = {threshold: [] for threshold in nnInfogain_thresholds}
    policy_accuracies_nInfogain = {threshold: [] for threshold in nnInfogain_thresholds}
    accuracies_nInfogain = {threshold: [] for threshold in nnInfogain_thresholds}
    # Predicted by true
    cm100_nInfogain = {threshold: [[0, 0], [0, 0]] for threshold in nnInfogain_thresholds}  # actual positive is if optimality = 100%/99%
    cm95_nInfogain = {threshold: [[0, 0], [0, 0]] for threshold in nnInfogain_thresholds}  # actual positive is if optimality = 95%/96%
    cm90_nInfogain = {threshold: [[0, 0], [0, 0]] for threshold in nnInfogain_thresholds}  # actual positive is if optimality = 90%/92%
    cm5_nInfogain = {threshold: [[0, 0], [0, 0]] for threshold in nnInfogain_thresholds}  # actual positive is with nEVD threshold = 0.5
    cm4_nInfogain = {threshold: [[0, 0], [0, 0]] for threshold in nnInfogain_thresholds}  # actual positive is with nEVD threshold = 0.4
    cm3_nInfogain = {threshold: [[0, 0], [0, 0]] for threshold in nnInfogain_thresholds}  # actual positive is with nEVD threshold = 0.3
    cm2_nInfogain = {threshold: [[0, 0], [0, 0]] for threshold in nnInfogain_thresholds}  # actual positive is with nEVD threshold = 0.2
    cm1_nInfogain = {threshold: [[0, 0], [0, 0]] for threshold in nnInfogain_thresholds}  # actual positive is with nEVD threshold = 0.1
    ...
    for demonstration in range(num_demonstration):
        ...
        nInfogain = compute_norm_infogain(env,
        ...
        info_gain_per_demo[demonstration].append(nInfogain)
        logger.info(f"Information gain {demonstration + 1} demonstrations: {nInfogain :.6f}")
        
        # evaluate thresholds
        for t in range(len(nnInfogain_thresholds)):
            threshold = nnInfogain_thresholds[t]
            ...
            if nInfogain <= threshold:
                num_demos_nInfogain[threshold].append(demonstration + 1)
                pct_states_nInfogain[threshold].append((demonstration + 1) / (env.rows * env.columns))
                policy_optimalities_nInfogain[threshold].append(optimality)
                policy_accuracies_nInfogain[threshold].append(calculate_policy_accuracy(policies[i], map_policy))
                accuracies_nInfogain[threshold].append(optimality >= optimality_threshold)
                curr_map_pi = map_policy

                # Evaluate actual positive by optimality
                if optimality >= 1.0:
                    cm100_nInfogain[threshold][0][0] += 1
                else:
                    cm100_nInfogain[threshold][0][1] += 1
                if optimality >= 0.95:
                    cm95_nInfogain[threshold][0][0] += 1
                else:
                    cm95_nInfogain[threshold][0][1] += 1
                if optimality >= 0.90:
                    cm90_nInfogain[threshold][0][0] += 1
                else:
                    cm90_nInfogain[threshold][0][1] += 1
                # Evaluate actual positive by nEVD
                if actual_nevd < 0.5:
                    cm5_nInfogain[threshold][0][0] += 1
                else:
                    cm5_nInfogain[threshold][0][1] += 1
                if actual_nevd < 0.4:
                    cm4_nInfogain[threshold][0][0] += 1
                else:
                    cm4_nInfogain[threshold][0][1] += 1
                if actual_nevd < 0.3:
                    cm3_nInfogain[threshold][0][0] += 1
                else:
                    cm3_nInfogain[threshold][0][1] += 1
                if actual_nevd < 0.2:
                    cm2_nInfogain[threshold][0][0] += 1
                else:
                    cm2_nInfogain[threshold][0][1] += 1
                if actual_nevd < 0.1:
                    cm1_nInfogain[threshold][0][0] += 1
                else:
                    cm1_nInfogain[threshold][0][1] += 1
            else:
                # Evaluate actual positive by optimality
                if optimality >= 1.0:
                    cm100_nInfogain[threshold][1][0] += 1
                else:
                    cm100_nInfogain[threshold][1][1] += 1
                if optimality >= 0.95:
                    cm95_nInfogain[threshold][1][0] += 1
                else:
                    cm95_nInfogain[threshold][1][1] += 1
                if optimality >= 0.90:
                    cm90_nInfogain[threshold][1][0] += 1
                else:
                    cm90_nInfogain[threshold][1][1] += 1
                # Evaluate actual positive by nEVD
                if actual_nevd < 0.5:
                    cm5_nInfogain[threshold][1][0] += 1
                else:
                    cm5_nInfogain[threshold][1][1] += 1
                if actual_nevd < 0.4:
                    cm4_nInfogain[threshold][1][0] += 1
                else:
                    cm4_nInfogain[threshold][1][1] += 1
                if actual_nevd < 0.3:
                    cm3_nInfogain[threshold][1][0] += 1
                else:
                    cm3_nInfogain[threshold][1][1] += 1
                if actual_nevd < 0.2:
                    cm2_nInfogain[threshold][1][0] += 1
                else:
                    cm2_nInfogain[threshold][1][1] += 1
                if actual_nevd < 0.1:
                    cm1_nInfogain[threshold][1][0] += 1
                else:
                    cm1_nInfogain[threshold][1][1] += 1
    ...
    ########################################################################
    # Store results for nInfogain
    ########################################################################
    num_demos_nInfogain_all.append(num_demos_nInfogain)
    pct_states_nInfogain_all.append(pct_states_nInfogain)
    policy_optimalities_nInfogain_all.append(policy_optimalities_nInfogain)
    policy_accuracies_nInfogain_all.append(policy_accuracies_nInfogain)
    accuracies_nInfogain_all.append(accuracies_nInfogain)
    cm100_nInfogain_all.append(cm100_nInfogain)
    cm95_nInfogain_all.append(cm95_nInfogain)
    cm90_nInfogain_all.append(cm90_nInfogain)
    cm5_nInfogain_all.append(cm5_nInfogain)
    cm4_nInfogain_all.append(cm4_nInfogain)
    cm3_nInfogain_all.append(cm3_nInfogain)
    cm2_nInfogain_all.append(cm2_nInfogain)
    cm1_nInfogain_all.append(cm1_nInfogain)    
    ...
    if (i+1)%2 == 0:
        ...
        np.save(os.path.join(save_dir, 'num_demos_nInfogain_all.npy'), num_demos_nInfogain_all)
        np.save(os.path.join(save_dir, 'pct_states_nInfogain_all.npy'), pct_states_nInfogain_all)
        np.save(os.path.join(save_dir, 'policy_optimalities_nInfogain_all.npy'), policy_optimalities_nInfogain_all)
        np.save(os.path.join(save_dir, 'policy_accuracies_nInfogain_all.npy'), policy_accuracies_nInfogain_all)
        np.save(os.path.join(save_dir, 'accuracies_nInfogain_all.npy'), accuracies_nInfogain_all)
        np.save(os.path.join(save_dir, 'cm100_nInfogain_all.npy'), cm100_nInfogain_all)
        np.save(os.path.join(save_dir, 'cm95_nInfogain_all.npy'), cm95_nInfogain_all)
        np.save(os.path.join(save_dir, 'cm90_nInfogain_all.npy'), cm90_nInfogain_all)
        np.save(os.path.join(save_dir, 'cm5_nInfogain_all.npy'), cm5_nInfogain_all)
        np.save(os.path.join(save_dir, 'cm4_nInfogain_all.npy'), cm4_nInfogain_all)
        np.save(os.path.join(save_dir, 'cm3_nInfogain_all.npy'), cm3_nInfogain_all)
        np.save(os.path.join(save_dir, 'cm2_nInfogain_all.npy'), cm2_nInfogain_all)
        np.save(os.path.join(save_dir, 'cm1_nInfogain_all.npy'), cm1_nInfogain_all)
