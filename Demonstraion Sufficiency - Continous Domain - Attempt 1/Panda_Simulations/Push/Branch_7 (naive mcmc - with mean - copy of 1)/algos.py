...
def softmax(numbers):
    ...
    sum_exp_numbers = np.sum(exp_numbers)
    return exp_numbers / sum_exp_numbers
...
def mcmc_naive(env, D, n_outer_samples, n_burn, len_theta, beta, step_size, normalize=False):
    
    theta = np.zeros(len_theta)
    p_theta = 0
    
    for xi in D:
        expR = p_xi_theta(env, xi, theta, beta)
        p_theta += expR
    ...
    for _ in range(n_outer_samples):
        ...
        p_theta1 = 0
        for xi in D:
            expR1 = p_xi_theta(env, xi, theta1, beta)
            p_theta1 += expR1
        ...
        elif  p_theta1 - p_theta > np.log(np.random.rand()):
