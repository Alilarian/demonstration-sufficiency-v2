...
for experiment_num in range(n_runs):
    ...
    random_indices = np.random.choice(len(pairwise_comparisons), num_demonstrations, replace=False)
    #trajectory_space = main_trajectory_space[random_indices]
    
    demonstrations = pairwise_comparisons[random_indices]
    ...
    for demonstration in range(num_demonstrations):
        ...
        pbirl = PBIRL(demonstrations[:demonstration+1], num_features=3, beta=beta, \
                      mcmc_samples=n_outer_samples, step_size=step_size)
        pbirl.run_mcmc()
