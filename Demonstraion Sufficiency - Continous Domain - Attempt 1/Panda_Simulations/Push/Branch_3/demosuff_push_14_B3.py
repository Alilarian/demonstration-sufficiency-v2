...
Reuslt:
- After 15 iterations the regret dcrease very noisy
- the true is larger (2 times)
- the true does not decrease
- Finally I think it does not work
"""
...
def trajectory_optimization(env, reward_param):
    """
    ...
    # generate many traj based on that reward and pick a good one
    _, best_xi, _, best_fs, best_reward = human_demo_2(env, xi0, reward_param, n_samples=200, n_demos=10)
    
    return best_xi, best_fs, best_reward

start_position = np.array([0.4, 0.0, 0.25])
goal_position = np.array([0.75, 0.0, 0.1])
xi0 = np.linspace(start_position, goal_position, 4)

true_reward_param = np.array([0.9701425 , 0.24253563])
...
demonstration_space, xi_star, trajectory_space, f_star, best_reward = human_demo_2(env, xi0, true_reward_param,\
                                                                           n_samples=num_generated_demos, n_demos=50)

all_demonstrations = np.array(demonstration_space)[:5]
...
for experiment_num in range(n_runs):
    ...
    avar_bounds_max_norm_map_policy = {k: {i: [] for i in range(1, 5)} for k in alphas}

    true_avar_bounds_max_norm_map_policy = {i: [] for i in range(1, 5)}


    avar_bounds_max_norm_mean_policy = {k: {i: [] for i in range(1, 5)} for k in alphas}

    true_avar_bounds_max_norm_mean_policy = {i: [] for i in range(1, 5)}
    ...
    for demonstration in range(num_demonstrations):
        ...
        best_traj_for_map, best_traj_feat_for_map, _  = \
            trajectory_optimization(env, reward_param=map_solution)


        #best_traj_for_mean, best_traj_feat_for_mean = \
        ...
        for i, mcmc_sample in enumerate(all_double_mcmc_sample):
            print(mcmc_sample)
            best_traj_for_current_sample, _, best_traj_reward_for_current_sample = \
                trajectory_optimization(env, reward_param=list(mcmc_sample))
    ...
    mean_results_map = {k: {i: [] for i in range(1, 5)} for k in alphas}

    true_mean_results_map = {1:[], 2:[], 3:[], 4:[]}
    ...
    if experiment_num%2 == 0:
        np.save('map_trajectories_B3.npy', np.array(map_trajs))
        np.save('optimal_trajectory_B3.npy', xi_star)