...
# Saving the best and worst trajectory based on the reward
np.save('results/best_traj_B12.2.npy', demonstration_space[0])
np.save('results/worst_traj_B12.2.npy', demonstration_space[-1])

all_demonstrations = np.array(demonstration_space)[:15]
...
acceptance_ratio_per_samples = {i: [] for i in range(0, 15)}
...
for experiment_num in range(n_runs):
    print("=================================================  Experiment Panda Push Branch B12.2 %d =================================================" %experiment_num)

    #avar_bounds_random_norm = {k: {i: [] for i in range(1, 16)} for k in alphas}

    #true_avar_bounds_random_norm = {i: [] for i in range(1, 16)}
    avar_bounds_max_norm_map_policy = {k: {i: [] for i in range(1, 16)} for k in alphas}

    true_avar_bounds_max_norm_map_policy = {i: [] for i in range(1, 16)}


    avar_bounds_max_norm_mean_policy = {k: {i: [] for i in range(1, 16)} for k in alphas}

    true_avar_bounds_max_norm_mean_policy = {i: [] for i in range(1, 16)}
    ...
    mean_results_map = {k: {i: [] for i in range(1, 16)} for k in alphas}

    true_mean_results_map = {i: [] for i in range(1, 16)}
    
    mean_results_mean = {k: {i: [] for i in range(1, 16)} for k in alphas}

    true_mean_results_mean = {i: [] for i in range(1, 16)}
    ...
    if experiment_num%20 == 0:
        np.save('results/acceptance_ratio_per_samples_B12.2.npy', acceptance_ratio_per_samples)
        np.save('results/mcmc_samples_B12.2.npy', mcmc_samples_all_experiments)
        np.save('results/map_trajectories_B12.2.npy', np.array(map_trajs))
        np.save('results/mean_trajectories_B12.2.npy', np.array(mean_trajs))
        np.save('results/optimal_trajectory_B12.2.npy', xi_star)
        np.save('results/avar_all_iterations_max_norm_map_policy_B12.2.npy', np.array(avar_all_iterations_max_norm_map_policy))
        np.save('results/true_avar_all_iterations_max_norm_map_policy_B12.2.npy', np.array(true_avar_all_iterations_max_norm_map_policy))