...
class Env1():
    ...
    def play_traj(self, xi, color, T=2.0):
    ...
    def feature_count(self, xi, color=[0, 0, 0]):
        n, m = np.shape(xi)
        height_reward = 0
        for idx in range(1, n):
            height_reward -= abs(xi[idx, -1])
        door_position = self.play_traj(xi, color)
        
        # this line penelize the robot if it pushes the door such that it passes the frame
        # then we should penelaize that
        # Door position is > 0 when it passes the frame
        pass_frame = 0
        if door_position > 0:
            pass_frame = -1
        else:
        ...
        f = np.array([door_position, height_reward, pass_frame])
        return f
