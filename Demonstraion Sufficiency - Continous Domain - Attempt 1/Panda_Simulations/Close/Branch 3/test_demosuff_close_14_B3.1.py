...
all_demonstrations, xi_star, main_trajectory_space, f_star = human_demo_2(env, xi0, true_reward_param , n_samples=num_generated_demos, n_demos=50)
...
for experiment_num in range(n_runs):
    print("=================================================  Experiment Panda Close Branch 3.0.5 %d =================================================" %experiment_num)
    ...
    for demonstration in range(num_demonstrations):
        ...
        all_double_mcmc_sample, map_solution, accept_ratio = mcmc_double_2(env, demonstrations[:demonstration + 1], n_outer_samples, n_inner_samples, n_burn, len(true_reward_param), beta=beta, step_size=step_size)
        ...
        #nevd_random_norm_map_policy = []
        #nevd_random_norm_mean_policy = []
        ...
        for i, mcmc_sample in enumerate(all_double_mcmc_sample):
            ...
            print("Best reward - sample  :%s, traj: %s, reward: %f\n" %(mcmc_sample, best_traj_for_current_sample, best_traj_reward_for_current_sample))
            
            print("Best reward MEAN - sample  :%s, traj: %s, reward: %f\n" %(mcmc_sample, best_traj_for_mean, mean_policy_reward_under_current_mcmc_sample))
            
            print("Best reward MAP - sample  :%s, traj: %s, reward: %f\n" %(mcmc_sample, best_traj_for_map, map_policy_reward_under_current_mcmc_sample))
            
            
            print("Regret: ", (best_traj_reward_for_current_sample - map_policy_reward_under_current_mcmc_sample) / (best_traj_reward_for_current_sample))
            ...
            nevd_max_norm_map_policy.append((best_traj_reward_for_current_sample - map_policy_reward_under_current_mcmc_sample) / (5))
            nevd_max_norm_mean_policy.append((best_traj_reward_for_current_sample - mean_policy_reward_under_current_mcmc_sample) / (5))
        ...
        true_avar_max_norm_map_policy = ((best_traj_true_reward - map_traj_true_reward) / (5))
        ...
        true_avar_max_norm_mean_policy = ((best_traj_true_reward - mean_traj_true_reward) / (5))
